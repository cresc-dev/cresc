...
public class UpdateModuleImpl {
    ...
    public static final String NAME = "Cresc";
            ...
            public void run() {
                try {
                    ...
                    try {
                        ...
                        promise.reject("cresc:"+err.getMessage());
                    ...
                    promise.reject("cresc:switchVersion failed"+err.getMessage());
                    Log.e("cresc", "switchVersion failed", err);
                }
            }
        });
    }
    ...
    public static void setNeedUpdate(UpdateContext updateContext, ReadableMap options,Promise promise) {
        try {
            final String hash = options.getString("hash");
            if(hash==null || hash.isEmpty()){
                promise.reject("hash can not be empty");
                return;
            }
            UiThreadUtil.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    try {
                        ...
                        Log.e("cresc", "switchVersionLater failed", err);
                    }
                }
            });
        } catch (Exception e){
            promise.reject("cresc:"+e.getMessage());
        }
    }
        ...
        UiThreadUtil.runOnUiThread(new Runnable() {
            ...
            public void run() {
                if(!check(info)){
                    ...
                    promise.reject("cresc: invalid json string");
                }else {
                    updateContext.setKv("hash_" + hash, info);
                    promise.resolve(true);
                }
